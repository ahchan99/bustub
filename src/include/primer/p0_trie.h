...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept
    ...
    for (auto &children_kv : other_trie_node.children_) {
      children_[children_kv.first] = std::move(children_kv.second);
    }
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (; i < key.size() - 1; i++) {
      if (!node->HasChild(key[i])) {
        node->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
      }
  ...
  bool Remove(const std::string &key) {
    ...
    auto j = static_cast<int>(node_array.size()) - 1;
    for (; j > 0; j--) {
      if (!node_array[j]->HasChildren() && !node_array[j]->IsEndNode()) {
        auto remove_key = node_array[j]->GetKeyChar();
