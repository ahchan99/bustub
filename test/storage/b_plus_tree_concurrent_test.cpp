...
// helper function to insert
void InsertHelper2(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> *tree, const std::vector<int64_t> &keys,
                   __attribute__((unused)) uint64_t thread_itr = 0) {
...
// helper function to seperate insert
void InsertHelperSplitSleep(BPlusTree<GenericKey<8>, RID, GenericComparator<8>> *tree, const std::vector<int64_t> &keys,
                            int total_threads, __attribute__((unused)) uint64_t thread_itr) {
  GenericKey<8> index_key;
  RID rid;
  // create transaction
...
// helper function to seperate insert
void InsertHelperSplit2(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> *tree, const std::vector<int64_t> &keys,
                        int total_threads, __attribute__((unused)) uint64_t thread_itr) {
  GenericKey<16> index_key;
  RID rid;
  // create transaction
...
// helper function to delete
void DeleteHelper2(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> *tree, const std::vector<int64_t> &remove_keys,
                   __attribute__((unused)) uint64_t thread_itr = 0) {
  GenericKey<16> index_key;
  // create transaction
  auto *transaction = new Transaction(0);
...
// helper function to seperate delete
void DeleteHelperSplit2(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> *tree,
                        const std::vector<int64_t> &remove_keys, int total_threads,
                        __attribute__((unused)) uint64_t thread_itr) {
  GenericKey<16> index_key;
  // create transaction
...
TEST(BPlusTreeConcurrentTest, InsertTest3) {
  ...
  LaunchParallelTest(30, InsertHelperSplitSleep, &tree, keys, 30);
...
TEST(BPlusTreeConcurrentTest, DeleteTest3) {
  ...
  InsertHelper2(&tree, keys);

  std::vector<int64_t> remove_keys;
  for (int i = 0; i <= (scale_factor - 20); i++) {
    remove_keys.push_back(i);
  }
  LaunchParallelTest(2, DeleteHelper2, &tree, remove_keys);
...
TEST(BPlusTreeConcurrentTest, DeleteTest4) {
  ...
  LaunchParallelTest(3, DeleteHelperSplit2, &tree, remove_keys, 3);
...
TEST(BPlusTreeConcurrentTest, MixTest2) {
  ...
  // concurrent insert
  LaunchParallelTest(4, InsertHelperSplit2, &tree, std::ref(all_deleted), 4);

  // concurrent insert and delete
  std::thread t0(InsertHelper2, &tree, keys, 0);
  LaunchParallelTest(4, DeleteHelperSplit2, &tree, std::ref(all_deleted), 4);
...
TEST(BPlusTreeConcurrentTest, MixTest3) {
  ...
  // concurrent insert
  LaunchParallelTest(4, InsertHelperSplit2, &tree, std::ref(deleted1), 4);
  LaunchParallelTest(4, InsertHelperSplit2, &tree, std::ref(deleted2), 4);
  // concurrent insert and delete
